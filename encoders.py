...
class Wav2Vec2Encoder(torch.nn.Module):
    ...
    def encode(self, raw_audio: Union[np.ndarray, List[float], List[np.ndarray], List[List[float]]]):
...
class EnCodecEncoder(torch.nn.Module):
    ...
    def encode(self, raw_audio: Union[np.ndarray, List[float], List[np.ndarray], List[List[float]]]):
        inputs = self.processor(raw_audio=raw_audio, sampling_rate=self.sampling_rate, return_tensors="pt").to(self.device)
